  ...
  class Asterisk_AMI
  {
    ...
    */
    function __construct($config=NULL, $optconfig=array())
    {
      ...
      if(!isset($this->config['asmanager']['username'])) $this->config['asmanager']['username'] = 'snep';
      if(!isset($this->config['asmanager']['secret'])) $this->config['asmanager']['secret'] = 'snep';
    }
    ...
    */
     ...
     * processEvent($event, $arrayParam);
     */
    function wait_event($obj) {
        $timeout = false;
      do
      {
        $type = NULL;
        ...
        switch($type)
        {
          case '': // timeout occured
            ...
            break;
          case 'event':
            $done = $obj->processEvent(strtolower($parameters['Event']),$parameters);
            break;
          default:
            $this->log('Unhandled response packet from Manager: ' . print_r($parameters, true));
            break;
