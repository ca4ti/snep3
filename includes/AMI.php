<?php
    ...
    class AMI
    {
        ...
        public function get_SIPshowregistry($host = false, $user=false)
        {
            ...
            if ($host && $user) {
                foreach ( $cmd_return as $key => $value) {
                    if ($value['host'] === $host && $value['username'] === $user) {
                        $show_registry = $cmd_return[$key] ;
                    }
                }
            } else {
                $show_registry = $cmd_return ;
            }
        ...
        public function get_IAXregistry($host = false, $user=false)
        {
            /* Get a list of all iax registry peers from asterisk
             *
             *  @return values:
             *      - Array of peers on success. Each peer is represented by an array containing all data of the peer.
             *        Instead of an 0-based array, the keys in the array is the peer name, for easier
            ...
            if ($host && $user) {
                foreach ( $cmd_return as $key => $value) {
                    if (substr($value['host'],0,strpos($value['host'], ":")) === $host && $value['username'] === $user) {
                        $show_registry = $cmd_return[$key] ;
                    }
                }
            } else {
