...
class Snep_Modules {
    ...
    /**
     * getRegisteredModules - Return and array of desriptors for registered modules.
    ...
    /**
     * addPath - Adds a path to module loading
    ...
    /**
     * parseInfo - Parses the file info.xml into a module descriptor object
    ...
    /**
     * loadResouces - Parses recursively the resources xml into Snep_Acl
    ...
    protected function loadResources(SimpleXMLElement $resources, $parent = null, $menu_parent = null) {
        ...
        foreach ($resources as $element) {
            ...
            if ($element->getName() == "resource") {
                ...
                $explode = explode("_", str_replace("/", "_", $resname));

                if (!isset($explode[2]) || !$explode[2])
                    $explode[2] = "read";
                Snep_Modules::$resources[$explode[0]][$explode[1]][$explode[2]] = (string) $element->attributes()->label;
            }
            else {
                $resname = $parent;
            }

            if ($element->attributes()->label !== null && !$element->attributes()->remove) {
    ...
    /**
     * registerModule - Take care of module registering details
    ...
    protected function registerModule($path) {
        ...
        if (file_exists("$path/resources.xml")) {
            ...
            Snep_Modules::$modules[$descriptor->getModuleId()] = $descriptor;
